...
def create_advanced_chart(data, symbol):
    ...
    if {'Open', 'High', 'Low', 'Close'}.issubset(data.columns):
        fig.add_trace(go.Candlestick(x=data.index, open=data['Open'], high=data['High'], low=data['Low'], close=data['Close'],
                                     name='Price', increasing_line_color='#00ff88', decreasing_line_color='#ff4444'), row=1, col=1)

    # Moving averages
    mas = [('SMA_20', 'SMA 20'), ('SMA_50', 'SMA 50'), ('SMA_200', 'SMA 200')]
    ma_colors = ['#ff9500', '#007aff', '#5856d6']
    for i, (col, label) in enumerate(mas):
        if col in data.columns and not data[col].isna().all():
            fig.add_trace(go.Scatter(x=data.index, y=data[col], mode='lines', name=label, line=dict(color=ma_colors[i], width=1.5)), row=1, col=1)
    ...
    if bb_upper_col and bb_lower_col:
        fig.add_trace(go.Scatter(x=data.index, y=data[bb_upper_col], mode='lines', line=dict(color='rgba(200,200,200,0.5)', width=1), name='BB Upper', showlegend=False), row=1, col=1)
        fig.add_trace(go.Scatter(x=data.index, y=data[bb_lower_col], mode='lines', line=dict(color='rgba(200,200,200,0.5)', width=1), name='BB Lower', fill='tonexty', fillcolor='rgba(200,200,200,0.08)', showlegend=False), row=1, col=1)

    # Volume (separate row)
    if 'Volume' in data.columns:
        vol_colors = ['#00ff88' if (row['Close'] >= row['Open']) else '#ff4444' for _, row in data[['Open','Close']].iterrows()]
        fig.add_trace(go.Bar(x=data.index, y=data['Volume'], marker_color=vol_colors, name='Volume', opacity=0.7), row=2, col=1)
        if 'Volume_SMA' in data.columns:
            fig.add_trace(go.Scatter(x=data.index, y=data['Volume_SMA'], mode='lines', name='Vol SMA', line=dict(color='#ffffff', width=1)), row=2, col=1)

    # MACD
    macd_col = None
    if 'MACD' in data.columns:
        macd_col = 'MACD'
        fig.add_trace(go.Scatter(x=data.index, y=data['MACD'], mode='lines', name='MACD', line=dict(color='#007aff', width=1.5)), row=3, col=1)
    sig_col = 'MACD_signal' if 'MACD_signal' in data.columns else ('MACD_Signal' if 'MACD_Signal' in data.columns else None)
    if sig_col:
        fig.add_trace(go.Scatter(x=data.index, y=data[sig_col], mode='lines', name='Signal', line=dict(color='#ff9500', width=1.5)), row=3, col=1)
    if 'MACD_histogram' in data.columns:
        hist = data['MACD_histogram']
        hist_colors = ['#00ff88' if v >= 0 else '#ff4444' for v in hist]
        fig.add_trace(go.Bar(x=data.index, y=hist, marker_color=hist_colors, name='Histogram', opacity=0.6), row=3, col=1)

    # RSI and Stochastic
    if 'RSI' in data.columns:
        fig.add_trace(go.Scatter(x=data.index, y=data['RSI'], mode='lines', name='RSI', line=dict(color='#af52de', width=1.5)), row=4, col=1)
        # reference lines
    ...
    fig.update_layout(title=f'{symbol} - Complete Technical Analysis Dashboard', xaxis_rangeslider_visible=False, height=900, showlegend=True, template='plotly_dark', font=dict(size=10))

    # hide x tick labels for upper subplots
    for r in (1, 2, 3):
        fig.update_xaxes(showticklabels=False, row=r, col=1)

    # style volume axis (row 2)
